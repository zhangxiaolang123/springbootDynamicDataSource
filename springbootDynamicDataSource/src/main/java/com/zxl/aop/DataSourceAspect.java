...
import com.zxl.config.DynamicDataSourceHolder;
...
public class DataSourceAspect {
    ...
    //模拟操作从表的一些方法
    private static String[] queryMethods = {"query","select","getAll","find","getUserByID"};
    private static String[] updateMethods = {"insert","add","delete","update","addUser"};
     ...
     * */
    @Pointcut("@annotation(com.zxl.bean.annotation.DataSource) || @within(com.zxl.bean.annotation.DataSource) ")
    ...
    //---------------------------------基于AOP，拦截某种\某个类-------------------------------------------------------------
    @Pointcut("execution( * com.zxl.service.impl.*Impl.*(..))")
    ...
    @Pointcut("execution( * com.zxl.mapper.*Mapper.*(..))")
